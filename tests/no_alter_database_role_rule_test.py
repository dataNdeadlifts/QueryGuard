...
class TestNoAlterDatabaseRole:
    ...
    def test_check_method_2(self) -> None:
        rule = NoAlterDatabaseRole()
        statements = SQLParser.get_all_statements("EXEC sp_addrolemember 'test_role', 'test_user'; ")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_check_method_3(self) -> None:
        rule = NoAlterDatabaseRole()
        statements = SQLParser.get_all_statements("EXEC sp_droprolemember 'test_role', 'test_user';")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_server_role(self) -> None:
        rule = NoAlterDatabaseRole()
        statements = SQLParser.get_all_statements("ALTER SERVER ROLE test_role ADD MEMBER test_user;")
        rule.check(statements)
    ...
    def test_handle_match_method(self) -> None:
        rule = NoAlterDatabaseRole()
        statement = SQLParser.get_all_statements("ALTER ROLE test_role ADD MEMBER test_user; ")[0]
        with pytest.raises(RuleViolation):
