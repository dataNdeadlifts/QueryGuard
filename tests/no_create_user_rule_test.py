...
class TestNoCreateUser:
    def test_check_method_1(self) -> None:
        rule = NoCreateUser()
        statements = SQLParser.get_all_statements("CREATE USER [test_user] FROM LOGIN [test_login];")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_check_method_2(self) -> None:
        rule = NoCreateUser()
        statements = SQLParser.get_all_statements("EXEC sp_adduser 'test_login', 'test_user';")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_check_method_3(self) -> None:
        rule = NoCreateUser()
        statements = SQLParser.get_all_statements("EXEC sp_grantdbaccess 'test_login', 'test_user';")
        with pytest.raises(RuleViolation):
            rule.check(statements)
