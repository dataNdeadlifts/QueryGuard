...
class TestNoAlterDatabaseFiles:
    def test_check_method_1(self) -> None:
        ...
        statements = SQLParser.get_statements("ALTER DATABASE test_database ADD FILEGROUP test_filegroup;")
    ...
    def test_check_method_3(self) -> None:
        rule = NoAlterDatabaseFiles()
        statements = SQLParser.get_statements("ALTER DATABASE test_database REMOVE FILE test_file;")
        with pytest.raises(RuleViolation):
            rule.check(statements)
    ...
    def test_check_method_5(self) -> None:
        rule = NoAlterDatabaseFiles()
        statements = SQLParser.get_statements("DBCC SHRINKFILE (test_file, 1);")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_check_method_6(self) -> None:
        rule = NoAlterDatabaseFiles()
        statements = SQLParser.get_statements("DBCC SHRINKDATABASE (test_database, 10);")
        with pytest.raises(RuleViolation):
            rule.check(statements)

    def test_check_method_7(self) -> None:
        rule = NoAlterDatabaseFiles()
        statements = SQLParser.get_statements("DBCC SHOWCONTIG ('test_database.test_table');")
        rule.check(statements)

    def test_check_method_8(self) -> None:
        rule = NoAlterDatabaseFiles()
        statements = SQLParser.get_statements("ALTER DATABASE [test_database] SET READ_ONLY;")
        rule.check(statements)
